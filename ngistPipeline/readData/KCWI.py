...
def readCube(config):
    ...
    # Read KCWI-cube
    printStatus.running("Reading the KCWI cube")
    logging.info("Reading the KCWI cube: " + config["GENERAL"]["INPUT"])
    ...
    printStatus.updateDone(
        "Done reading " + str(len(cube["x"])) + " spectra from the KCWI cube"
    )

    logging.info(
        "Finished reading the KCWI cube! Read a total of "
        + str(len(cube["x"]))
        + " spectra!"
    )
